...
String pageHtml() {
  ...
  s += "a{color:var(--text)}";
  s += ".powerRow{display:flex;align-items:center;gap:12px;flex-wrap:wrap}";
  ...
  s += "<div class='powerRow'>";
  s += "<label class='switch'><input id='enabled' type='checkbox'><span class='slider'></span></label>";
  s += "<div><div class='powerLabel'>Humidifier Power</div><div class='muted' id='powerText'>Enabled</div></div>";
  s += "</div>";
  ...
  s += "function syncPowerText(){";
  s += "  $('powerText').textContent=$('enabled').checked?'Enabled':'Disabled';";
  s += "}";
  ...
  s += "  syncPowerText();";
  s += "  const r=await fetch(`/set?off=${offMs}&on=${onMs}&enabled=${enabled}`,{cache:'no-store'});";
...
String otaPageHtml() {
  ...
  s += "<h2>OTA Update</h2>";
  s += "<div class='card'>";
  s += "<div class='muted'><b>Warning:</b> Only upload firmware built for this ESP32-C3. Using the wrong <b>.bin</b> can fail or leave the device unable to boot.</div>";
  s += "<div class='muted'>Recommended <b>.bin</b> files and future updates: <a href='https://github.com/Crucial-hash/ESP32-Humidifier' target='_blank' rel='noopener noreferrer'>github.com/Crucial-hash/ESP32-Humidifier</a></div>";
